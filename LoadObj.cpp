/*
    C++ implementation of an OBJ loader by Dr. Mike Bailey
    Takes in an OBJ file, parses the vertices, normals, and texture coordinates data
    and displays them on the screen as a triangle mesh.

    In a perfect world, we could take these C++ functions, extract the algorithm, and
    translate its functionality into Rust. The OpenGL processes are the same in Rust and C++, 
    but of course there's other funcitonalities that need to be considered, and those are going
    to be what cause the actual issues.

    I pulled this code from my GitHub if you would like to look at a sample implementation 
    of this function: https://github.com/toddgr/CS450-Animate-A-Helicopter/blob/main/sample.cpp
    It's a rough implementation but it does the job
*/
